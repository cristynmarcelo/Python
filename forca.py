...
while ganhou == False: 
  ...
  print('\n')
  '''Se suas vidas acabarem esse if vai terminar
   o jogo ↓'''
  if vidas < 1:
  ...
  '''Loop caso o usuário tente fechar
     o programa↓'''
  while len(savle) > 1 or savle == '' or detect_espaço == True or detect_letra == False:
  ...
  if acertou < 1 and savle not in letras_usadas:
    print('\n' * 5)
  ...
  #LETRAS USADAS
  if savle in letras_usadas:
    print(f"- A letra '{savle.upper()}' já foi usada", "\n" * 5)
    sleep(3)
  else:  
    letras_usadas.append(savle)
